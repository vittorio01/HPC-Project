...
compile: libs-compile $(PRG)/main.c	
	...
	echo 'mpiexec --map-by core --bind-to core -n $(PROCESSES) $(OUT_FOLDER)/$(PRG) --bats $(BATS) --iterations $(ITERATIONS) --dim $(DIM) --alpha $(ALPHA) --gamma $(PULSE) --loudness $(LOUDNESS) --fmin $(FMIN) --fmax $(FMAX) --radius $(RADIUS) --function $(FUNCTION)' >> $(RUN)
	chmod +x $(RUN)
...
run-local: compile 
	...
	mpiexec --map-by core --bind-to core -n $(PROCESSES) $(OUT_FOLDER)/$(PRG) --bats $(BATS) --iterations $(ITERATIONS) --dim $(DIM) --alpha $(ALPHA) --gamma $(PULSE) --loudness $(LOUDNESS) --fmin $(FMIN) --fmax $(FMAX) --radius $(RADIUS) --function $(FUNCTION)
